...
class SmartBrowzClient {
    async generatePDF(htmlContent) { throw new Error("Method not implemented"); }
}
...
class MockCatalystDataStore extends DataStoreClient {
    ...
    async query(sql) {
        ...
        if (lower.includes("select mo_id from fir_mo_link")) {
            const matchFir = sql.match(/fir_id\s*=\s*'([^']+)'/i);
            const targetId = matchFir ? sanitizeZCQLString(matchFir[1]) : '';
            const list = this.adapter.dbData.tables.fir_mo_link || [];
            const filtered = matchFir ? list.filter(r => r.fir_id === targetId) : list;
            return filtered.map(row => ({ fir_mo_link: row }));
        }
...
class RealSmartBrowzClient extends SmartBrowzClient {
    constructor(app) {
        super();
        this.app = app;
    }
    ...
    async generatePDF(htmlContent) {
        // SmartBrowz HTML-to-PDF conversion API endpoint
        return await this.app.smartbrowz().convertToPDF({ html: htmlContent });
    }
}
...
class ClientAdapter {
    constructor(context) {
        ...
        if (useMock) {
            ...
            this.smartbrowz = new MockSmartBrowzClient();
        } else {
            ...
            this.smartbrowz = new RealSmartBrowzClient(app);
        }
    }

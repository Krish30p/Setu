...
class MockCatalystDataStore extends DataStoreClient {
    ...
    async query(sql) {
        ...
        if (lower.includes("select fir_id from offender_phone_link")) {
            const matchPhone = sql.match(/phone_id\s*=\s*'([^']+)'/i);
            const targetId = matchPhone ? sanitizeZCQLString(matchPhone[1]) : '';
            const list = this.adapter.dbData.tables.offender_phone_link || [];
            const filtered = matchPhone ? list.filter(r => r.phone_id === targetId) : list;
            return filtered.map(row => ({ offender_phone_link: row }));
        }
        if (lower.includes("select fir_id from offender_vehicle_link")) {
            const matchVeh = sql.match(/vehicle_id\s*=\s*'([^']+)'/i);
            const targetId = matchVeh ? sanitizeZCQLString(matchVeh[1]) : '';
            const list = this.adapter.dbData.tables.offender_vehicle_link || [];
            const filtered = matchVeh ? list.filter(r => r.vehicle_id === targetId) : list;
            return filtered.map(row => ({ offender_vehicle_link: row }));
        }
...
class MockQuickMLClient extends QuickMLClient {
    async executeLLM(endpointKey, prompt) {
        ...
        return "Mock RAG Answer Text based on query context.";
    }
}

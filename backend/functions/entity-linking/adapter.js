...
class DataStoreClient {
    ...
    async resolveNameMatch(name, candidates) { throw new Error("Method not implemented"); }
}
...
class MockCatalystDataStore extends DataStoreClient {
    ...
    async resolveNameMatch(name, candidates) {
        return deterministicNameMatch(name, candidates);
    }
}
...
class RealCatalystDataStore extends DataStoreClient {
    ...
    async resolveNameMatch(name, candidates) {
        // In real cloud mode, this is parsed directly from QuickML responses in index.js
        return { is_likely_match: false, matched_offender_id: null, confidence: 0.0, reasoning_summary: 'Cloud parsing deferred' };
    }
}
